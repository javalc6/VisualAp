...
Version 1.2.3, 12-01-2025, Update to use github for beans check/update
...
//	static final String hostingWebServer = "https://visualap.sourceforge.net/"; DEPRECATED, TODO: implement new component management
	...
	static final String beansEndPoint = "https://api.github.com/repos/javalc6/VisualAp/contents/beans";
...
private class InsertBean implements callback {
	...
	void updateComponents(HashSet<String> updatel) {
		if (JOptionPane.OK_OPTION == JOptionPane.showConfirmDialog(VisualAp.this,
			...
			String datapath = prefs.get("dataPath", null); 
			try {
//note, code based on github endpoint cannot use parameter updatel, as cannot be used as key in beans folder
/* old code using deprecated hostingWebServer
...
*/
				String json = fetch.fetchContent(beansEndPoint);
				if (!json.isEmpty()) {
					JSONArray beans = (JSONArray) JSONValue.parse(json);
					int n_beans = beans.size();
					for (int i = 0; i < n_beans; i++) {
						JSONObject bean = (JSONObject) beans.get(i);
						String name = (String) bean.get("name").toJava();
//						String path = (String) bean.get("path").toJava();
						String sha = (String) bean.get("sha").toJava();
						String download_url = (String) bean.get("download_url").toJava();

						File localfile = new File(datapath+File.separatorChar+"beans"+File.separatorChar+name);
						String localSha = getGitBlobHash(localfile);
						if (localSha == null || !localSha.equals(sha)) {
							System.out.println("Downloading: " + download_url);
							fetch.fetchFile(download_url, new File(datapath+File.separatorChar+"beans"+File.separatorChar+name));
							downloadfiles.add(name);
						}
					}
					JOptionPane.showMessageDialog(VisualAp.this, 
						"Download completed\nExiting: restart VisualAp", "VisualAp", JOptionPane.INFORMATION_MESSAGE, icon);
					System.exit(0);
				} JOptionPane.showMessageDialog(VisualAp.this, "Technical problems\nCannot fetch information from beansEndPoint","Error",JOptionPane.ERROR_MESSAGE);
			}
    ...
    public static String getGitBlobHash(File file) throws IOException, NoSuchAlgorithmException {
        MessageDigest digest = MessageDigest.getInstance("SHA-1");
        long fileSize = file.length();
        String header = "blob " + fileSize + "\0";
        digest.update(header.getBytes());
...
// corrected fault: NodeText instead of NodeBean
				...
				for (int i = 0; i < beans.size(); i++) {
					if (beans.get(i).name.equals(action)) {
						try {
							Object myBean = beans.get(i).clazz.getDeclaredConstructor().newInstance();
